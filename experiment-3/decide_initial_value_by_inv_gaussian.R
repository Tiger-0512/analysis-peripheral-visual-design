...
# f <- glmer(reactionTime ~ log(stimSize) + size_1 + size_2 + pos + (1 + log(stimSize) + size_1 + size_2 + pos|id), data=dat, family=inverse.gaussian(link="identity"), control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))

plot(f1)  # Residual Error Plot
...
(5.3333 / 2)^2 * pi
t <- exp(1.192104 + 0.167547/22.339)


S <- 0.167547 / (log(3.3188) - 1.192104)
r <- sqrt(S/pi)
R <- 2 * r
