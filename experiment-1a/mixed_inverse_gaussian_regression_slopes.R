...
# Polynomial model
f2 <- glmer(reactionTime ~ poly(pos, 2, raw = TRUE) + arrange + poly(pos, 2, raw = TRUE):arrange + (1 + poly(pos, 2, raw = TRUE) + arrange|id), data = dat, family = inverse.gaussian(link="identity"), control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
